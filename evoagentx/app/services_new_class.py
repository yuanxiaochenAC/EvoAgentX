...
class WorkflowService:
    ...
    async def create_workflow(workflow_data: WorkflowCreate, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # # Process steps as WorkflowStepDefinition objects
        # steps = workflow_dict["definition"].get("steps", [])
        # for step in steps:
        #     # Assuming step is already a WorkflowStepDefinition object
        #     agent_id = step.agent_id
        #     # Validate agent exists
        #     agent = await AgentService.get_agent(agent_id)
        #     if not agent:
    ...
    async def delete_workflow(workflow_id: str) -> bool:
        ...
        if pending_executions:
            pending_ids = [exec["_id"] for exec in pending_executions]
            await Database.executions.delete_many({"_id": {"$in": pending_ids}})
            logger.info(f"Removed {len(pending_executions)} pending executions for workflow {workflow_id}")
