...
class AgentRunner:
    ...
    async def start(self):
        ...
        logger.info(f"Started agent {self.agent.name}")

    async def run(self):
        """Run the agent's main loop."""
        while self.running:
            try:
                # Execute agent's default action or monitoring loop
                await self.agent.execute("monitor", action_input_data={"status": "running"})
                await asyncio.sleep(5)
            except Exception as e:
                logger.error(f"Error in agent {self.agent.name}: {str(e)}")
                await asyncio.sleep(5)
    ...
    async def stop(self):
        ...
        if self.task:
            ...
            try:
                ...
                pass
        logger.info(f"Stopped agent {self.agent.name}")
...
class AgentService:
    ...
    @classmethod
    async def create_agent(cls, agent_data: AgentCreate, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            agent = Agent(
                ...
                llm_config=agent_dict.get("config", {}),
            )
            
            # Add to agent manager
            cls._agent_manager.add_agent(agent)
            
            # Create and start runner
            runner = AgentRunner(agent)
            cls._runners[agent_dict["name"]] = runner
            await runner.start()

            logger.info(f"Created and started agent {agent_dict['name']} with ID {result.inserted_id}")
    ...
    @classmethod
    async def get_agent_by_name(cls, name: str) -> Optional[Dict[str, Any]]:
        """Get an agent by name."""
        return await Database.agents.find_one({"name": name})
    
    @classmethod
    async def update_agent(cls, agent_id: str, agent_data: AgentUpdate) -> Optional[Dict[str, Any]]:
        """Update an agent's configuration and restart it if necessary."""
        if not ObjectId.is_valid(agent_id):
            raise ValueError(f"Invalid agent ID: {agent_id}")
        ...
        runner = cls._runners.get(agent["name"])
        if runner:
            await runner.stop()
            ...
            new_agent = Agent(
                ...
                llm_config=update_data.get("config", agent["config"]),
            )
            cls._agent_manager.remove_agent(agent["name"])
            cls._agent_manager.add_agent(new_agent)
            
            # Create and start new runner
            new_runner = AgentRunner(new_agent)
            cls._runners[agent["name"]] = new_runner
    ...
    async def delete_agent(cls, agent_id: str) -> bool:
        """Delete an agent and stop its execution."""
        if not ObjectId.is_valid(agent_id):
            raise ValueError(f"Invalid agent ID: {agent_id}")
    ...
    async def get_agent_status(cls, agent_id: str) -> Dict[str, Any]:
        ...
        status = {
            ...
            "state": cls._agent_manager.get_agent_state(agent["name"]) if cls._agent_manager.has_agent(agent["name"]) else None
        }
        return status
