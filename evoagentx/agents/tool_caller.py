...
from ..actions.tool_caller_action import ToolCallerAction
...
class ToolCaller(Agent):
    ...
    1. ToolCallerAction - Analyzes the query and decides what to do
    2. ToolCalling - Executes the selected tool if required
    """
    ...
    def __init__(self, **kwargs):
        ...
        # Initialize actions
        tool_caller_action = ToolCallerAction()
        tool_calling_action = ToolCalling()
        actions = kwargs.pop("actions", [tool_caller_action, tool_calling_action])
        ...
        # Store action names for convenience
        self.tool_caller_action_name = tool_caller_action.name
    ...
    async def execute(self, action_name: str, action_input_data: dict, return_msg_type: MessageType = MessageType.UNKNOWN) -> Message:
        ...
        """
        # If the action is tool_caller_action, add tool descriptions to the input
        if action_name == self.tool_caller_action_name:
            action_input_data["tool_descriptions"] = self.tool_descriptions
        ...
        # If the action was tool_caller_action and it returned a tool call,
        # execute the tool call as well
        if action_name == self.tool_caller_action_name:
            parser = message.content
            if parser.action == "tool_call":
                tool_call_info = parser.get_tool_call_info()
                if tool_call_info:
                    ...
                    if matching_tool and matching_tool.get("callable_fn"):
                        # Execute the tool call
                        tool_input_data = {
                            "callable_fn": matching_tool["callable_fn"],
                            "function_args": tool_parameters
                        }
                        message = await super().execute(
