...
class EvaluationUtils:
    ...
    async def evaluate_graph_async(self, optimizer, validation_n, data):
        ...
        for _ in range(validation_n):
            ...
            with suppress_logger_info():
                score, avg_cost, total_cost = await evaluator.graph_evaluate_async(optimizer.benchmark, optimizer.graph, is_test=False)
    ...
    async def evaluate_graph_test_async(self, optimizer):
        ...
        evaluator = AFlowEvaluator(llm=optimizer.executor_llm)
        with suppress_logger_info():
            score, avg_cost, total_cost = await evaluator.graph_evaluate_async(optimizer.benchmark, optimizer.graph, is_test=True)
