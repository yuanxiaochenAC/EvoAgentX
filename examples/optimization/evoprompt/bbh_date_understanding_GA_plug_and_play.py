...
from evoagentx.optimizers.evoprompt_optimizer import GAOptimizer, _split_mega_prompt_to_config
...
class DateUnderstandingProgram:
    def __init__(self, model: OpenAILLM):
        self.model = model
        self.instruction = "Based on the context and question, choose the answer."
...
async def main():
    ...
    benchmark = BIGBenchHard("date_understanding", sample_num=25)
    benchmark._load_data()

    program = DateUnderstandingProgram(model=llm)

    registry = ParamRegistry()
    registry.track(program, "instruction", name="date_instruction")

    logger.info("Starting EvoPrompt Optimizer for Date Understanding task...")
    ...
    best_instruction = final_config.get('date_instruction', 'Error: prompt not found.')

    logger.info(f"Initial instruction was: '{DateUnderstandingProgram(None).instruction}'")
